...
public class BeatsDecoder {
     ...
     */
    public BeatsDecoder(final Charset charset, final ByteArrayOutputStream buffer) {
        ...
        switch (decodedFrameType) {
            case FRAME_WINDOWSIZE: //'W'
                if (currBytes.size() < WINDOWSIZE_LENGTH ) {
                    logger.trace("Beats currBytes contents are {}", currBytes.toString());
                    break;
                    ...
                    logger.debug("Saw a packet I should not have seen. Packet contents were {}", currBytes.toString());
                    break;
                }
            case FRAME_COMPRESSED: //'C'
                if (currBytes.size() < COMPRESSED_MIN_LENGTH) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("Received a compressed frame with partial data or invalid content. The packet contents were {}", currBytes.toString());
                    }
                    break;
                }
            case FRAME_JSON: // 'J́'
